#[derive(Clone)]
pub struct ZoomLevel {
    pub tile_width: u16,
    pub tile_height: u16,
}

#[derive(Clone)]
pub struct Tilemap {
    pub zoom_levels: Vec<ZoomLevel>,
    pub current_zoom: usize,
    pub camera_x: i32,
    pub camera_y: i32,
}

impl Default for Tilemap {
    fn default() -> Self {
        Self::new()
    }
}

impl Tilemap {
    pub fn new() -> Self {
        Self {
            zoom_levels: vec![
                ZoomLevel { tile_width: 52, tile_height: 24 },
                ZoomLevel { tile_width: 24, tile_height: 12 },
                ZoomLevel { tile_width: 12, tile_height: 6 },
                ZoomLevel { tile_width: 2, tile_height: 1 },
            ],
            current_zoom: 1, // Medium
            camera_x: 0,
            camera_y: 0,
        }
    }

    pub fn zoom_in(&mut self, offset: Option<(i32, i32)>) {
        if self.current_zoom > 0 {
            let old_tw = self.zoom_levels[self.current_zoom].tile_width as f32;
            let old_th = self.zoom_levels[self.current_zoom].tile_height as f32;
            self.current_zoom -= 1;
            let new_tw = self.zoom_levels[self.current_zoom].tile_width as f32;
            let new_th = self.zoom_levels[self.current_zoom].tile_height as f32;
            let rx = new_tw / old_tw;
            let ry = new_th / old_th;
            let (ox, oy) = offset.unwrap_or((0, 0));
            self.camera_x = (self.camera_x as f32 * rx + ox as f32 * (rx - 1.0)).round() as i32;
            self.camera_y = (self.camera_y as f32 * ry + oy as f32 * (ry - 1.0)).round() as i32;
        }
    }

    pub fn zoom_out(&mut self, offset: Option<(i32, i32)>) {
        if self.current_zoom < self.zoom_levels.len() - 1 {
            let old_tw = self.zoom_levels[self.current_zoom].tile_width as f32;
            let old_th = self.zoom_levels[self.current_zoom].tile_height as f32;
            self.current_zoom += 1;
            let new_tw = self.zoom_levels[self.current_zoom].tile_width as f32;
            let new_th = self.zoom_levels[self.current_zoom].tile_height as f32;
            let rx = new_tw / old_tw;
            let ry = new_th / old_th;
            let (ox, oy) = offset.unwrap_or((0, 0));
            self.camera_x = (self.camera_x as f32 * rx + ox as f32 * (rx - 1.0)).round() as i32;
            self.camera_y = (self.camera_y as f32 * ry + oy as f32 * (ry - 1.0)).round() as i32;
        }
    }

    pub fn pan(&mut self, dx: i32, dy: i32) {
        self.camera_x += dx;
        self.camera_y += dy;
    }
}
